...
fn main_integration_test() {
    ...
    house.add_device(
        String::from("sportroom"),
        Rosette,
        String::from("Sport rosette"),
    );
    assert!(house
        .get_room_devices("sportroom")
        .unwrap()
        .get(&Rosette)
        .unwrap()
        .contains("Sport rosette"));

    house.remove_device("kitchen", &Devices::Rosette, "Left");

    assert!(!house
        .get_room_devices("kitchen")
        .unwrap()
        .get(&Rosette)
        .unwrap()
        .contains("Left"));
